...
func renderBar(lastTime time.Time, downloaded *int64, total, lastBytes int64, width int) {
		now := time.Now()
		done := atomic.LoadInt64(downloaded)

		if total <= 0 { return } // Hindari pembagian nol
...
func ShowProgress(total int64, downloaded *int64, doneDownload chan bool) {
